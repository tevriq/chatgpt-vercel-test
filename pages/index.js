...
export default function Home(){
  ...
  const src=mode==='interactive'?'/api/prototype':mode==='gallery'?'/api/gallery':'/api/motion-v3';
  const title=mode==='interactive'?'P14 iOS 27 r6 interactive review':mode==='gallery'?'P14 iOS 27 r6 high fidelity gallery':'P14 semantic continuity motion v3';
  const bg=mode==='motion'?'#07080d':mode==='interactive'?'#f2f2f7':'#ececf1';
  return <div style={{position:'fixed',inset:0,background:'#dedee3',fontFamily:'-apple-system,BlinkMacSystemFont,"SF Pro Display","PingFang SC",sans-serif'}}>
    <div style={{position:'fixed',top:14,left:'50%',transform:'translateX(-50%)',zIndex:50,display:'flex',gap:4,padding:4,borderRadius:14,background:'rgba(255,255,255,.9)',boxShadow:'0 8px 28px rgba(0,0,0,.14)',backdropFilter:'blur(18px)'}}>
      <button onClick={()=>setMode('interactive')} style={btn(mode==='interactive')}>交互预览 · r6</button>
      <button onClick={()=>setMode('gallery')} style={btn(mode==='gallery')}>iOS 27 UI 图册</button>
      <button onClick={()=>setMode('motion')} style={btn(mode==='motion')}>语义动效 v3</button>
    </div>
